...
from .views.views_menu import MenuCalendarView, MenuView, delete_menu, copy_menu
...
urlpatterns = [
    ...
    url(r'^calculations/(?P<year>\d{4})[.-]{1}(?P<month>\d{2})[.-]{1}(?P<day>\d{2})'
         '\/(?P<childrens>\d{1,3})$',
    ...
    url(r'^dish/(?P<pk>\d+)/delete/?$', DeleteDishView.as_view(), name='dish-delete'),
    ...
    url(r'^map/(?P<pk>\d+)/delete/?$', DeleteMapView.as_view(), name='map-delete'),
