...
def get_report_calculation_of_day(day,qty_children):
    ...
    for item in Menu.objects.filter(created_at=day):
        ...
        if dish_ingredients: 
            ...
            ingredients = [dict(zip(keys_row,
                                    [ingredient.product,
                                     ingredient.netto / 1000 * qty_children,
                                     ingredient.netto, 0]))
        ...
        products +=[ingredient[keys_row[0]] for ingredient in ingredients 
        ...
        for product in products:
            for dish_ingredient in item_dish['ingredients']:
                if dish_ingredient[keys_row[0]] == product:
                    total_weigth[product] += float(dish_ingredient[keys_row[1]])
    ...
    return {'calculation_day':tabl, 'products':products,'all_total':all_total, 'total_per_child': all_total / qty_children}
